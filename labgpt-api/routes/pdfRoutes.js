...
// IMPROVED MARKDOWN TO PDF CONVERTER WITH TIMEOUT FIXES
async function markdownToPDF(markdownText, filename = 'document.pdf') {
  log.debug('📝 Starting markdown to PDF conversion using Puppeteer');
  ...
  try {
    ...
    // Launch browser with optimized configuration for Docker
    browser = await puppeteer.launch({
      headless: true,
      args: [
        '--no-sandbox',
    ...
    // Intercept network requests to handle potential image loading issues
    await page.setRequestInterception(true);
    page.on('request', (request) => {
      const url = request.url();
      
      // Allow data URLs and the specific logo
      if (url.startsWith('data:') || url.includes('labmatelogo.png')) {
        request.continue();
      } else if (url.startsWith('https://labmate.docspace.co.ke/')) {
        // Set a timeout for external resources
        request.continue();
      } else {
        // Block other external resources to prevent hanging
        request.abort();
      }
    });
    ...
    page.on('response', response => {
      if (!response.ok()) {
        log.warn(`Failed to load resource: ${response.url()} - ${response.status()}`);
      }
    });
    
    // Set content with reduced wait conditions
    await page.setContent(htmlContent, { 
      waitUntil: 'domcontentloaded', // Changed from 'networkidle0' to be less strict
      timeout: 45000 // 45 second timeout
    });
    
    // Wait a bit for any remaining resources, but don't block
    try {
      await page.waitForTimeout(2000); // Give images 2 seconds to load
    } catch (err) {
      log.warn('Timeout waiting for additional resources, proceeding anyway');
    }
    ...
    if (browser) {
      try {
        ...
        log.warn('Error closing browser:', closeError);
      }
    }
  }
}

// ALTERNATIVE: FALLBACK HTML GENERATION WITHOUT EXTERNAL RESOURCES
function generateHTMLWithoutExternalResources(markdownText, testInfo, filename) {
  const processedContent = processMarkdownContent(markdownText, testInfo);
  const css = generateCSS(testInfo.format, testInfo.testType);
  const currentDate = new Date().toLocaleDateString('en-US', { 
    year: 'numeric', 
  ...
  // Use base64 encoded logo or remove it entirely to avoid network issues
  const logoBase64 = 'data:image/svg+xml;base64,';
...
<body>
  <div class="header">
    <img src="${logoBase64}" alt="LabMate Logo" style="max-height: 70px; margin-bottom: 15px;">
    <h1>${testInfo.testType}</h1>
...
// UPDATED GENERATE FUNCTION WITH BETTER ERROR HANDLING
function generateHTML(markdownText, testInfo, filename) {
  try {
    // First try with external resources
    return generateHTMLWithExternalResources(markdownText, testInfo, filename);
  } catch (error) {
    log.warn('Falling back to HTML without external resources due to error:', error);
    // Fallback to version without external resources
    return generateHTMLWithoutExternalResources(markdownText, testInfo, filename);
  }
}

function generateHTMLWithExternalResources(markdownText, testInfo, filename) {
  const processedContent = processMarkdownContent(markdownText, testInfo);
  const css = generateCSS(testInfo.format, testInfo.testType);
  const currentDate = new Date().toLocaleDateString('en-US', { 
    year: 'numeric', 
...
<body>
  <div class="header">
    <img src="https://labmate.docspace.co.ke/labmatelogo.png" alt="LabMate Logo" style="max-height: 70px; margin-bottom: 15px;" onerror="this.style.display='none';">
    <h1>${testInfo.testType}</h1>
    <div class="subtitle">Generated on ${currentDate} | Format: ${testInfo.format.charAt(0).toUpperCase() + testInfo.format.slice(1)}</div>
  </div>
  <div class="content">
